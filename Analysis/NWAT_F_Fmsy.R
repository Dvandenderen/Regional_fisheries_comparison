...
NWAT <- ggplot() +
  ...
  geom_ribbon(data = avg_tr, aes(x = Year, ymin = lower, ymax = upper), fill = "blue", alpha = 0.2) +  # CI ribbon
  geom_line(data = avg_tr, aes(x = Year, y = mean_F), color = "blue", size = 1.2) +  # mean line
  theme(legend.position = "none")+theme_classic()+coord_cartesian(ylim = c(0, 4.5))+labs(title="B) NW Atlantic")

rm(list=setdiff(ls(), c("NWEU","NWAT")))